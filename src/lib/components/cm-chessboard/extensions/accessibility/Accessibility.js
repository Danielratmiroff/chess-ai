...
export class Accessibility extends Extension {
	constructor(chessboard, props) {
		...
		if (this.props.movePieceForm) {
			...
			this.form.addEventListener('submit', (evt) => {
				evt.preventDefault();
				if (
					this.chessboard.view.moveInputCallback({
		...
		if (this.props.boardAsTable) {
			this.boardAsTableContainer = this.createElement(
				`<div><h3>${this.th.board_as_table}</h3><div class="table"></div></div>`
			);
		...
		if (this.props.piecesAsList) {
			this.piecesListContainer = this.createElement(
				`<div><h3>${this.th.pieces_lists}</h3><div class="list"></div></div>`
			);
	...
	redrawPositionInAltAttribute() {
		...
		for (const piece of pieces) {
			const pieceName =
				piece.name === 'p' ? '' : piecesTranslations[this.lang].pieces[piece.name].toUpperCase();
			if (piece.color === 'w') {
...
${listB}`;
	...
	redrawPiecesLists() {
		...
		for (const piece of pieces) {
			if (piece.color === 'w') {
				listW += `<li class="list-inline-item">${renderPieceTitle(this.lang, piece.name)} ${
					piece.position
				}</li>`;
			} else {
				listB += `<li class="list-inline-item">${renderPieceTitle(this.lang, piece.name)} ${
					piece.position
				}</li>`;
			}
		}
