...
export class ChessboardView {
	constructor(chessboard) {
		...
		this.moveInput = new VisualMoveInput(
			this,
	...
	createSvgAndGroups() {
		...
		this.svg.setAttribute(
			'class',
			'cm-chessboard border-type-' + this.chessboard.props.style.borderType + ' ' + cssClass
		);
	...
	updateMetrics() {
		...
		this.pieceXTranslate =
			this.squareWidth / 2 - (this.chessboard.props.sprite.size * this.scalingY) / 2;
	}

	handleResize() {
		this.context.style.width = this.chessboard.context.clientWidth + 'px';
		this.context.style.height =
			this.chessboard.context.clientWidth * this.chessboard.props.style.aspectRatio + 'px';
	...
	moveInputStartedCallback(square) {
		...
		const extensionPointsResult = this.chessboard.state.invokeExtensionPoints(
	...
	visualizeInputState() {
		if (this.chessboard.state) {
			...
			if (
				...
				this.chessboard.state.squareSelectEnabled
			) {
